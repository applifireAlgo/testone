Ext.define('Shoppingportal.shoppingportal.web.com.shopping.controller.onlineshopping.CartMainController', {
     extend: 'Shoppingportal.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.CartMainController',
     init: function() {
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#CartTree');
               ...
               var compRef = this.view.down('#CartTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.cartId;
          me = this;
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Cart');
          var grid = this.view.down('#CartGrid');
          var tree = this.view.down('#CartTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#CartTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var Cart = this.view.down('#CartForm');
               Cart.reset();
               var gridPanel = this.view.down('#CartGrid');
          ...
          var formPanel = this.getView().up().down('#CartForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().down('#CartTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Cart').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CartGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#CartForm').getViewModel().setData(responseData.response.data);
     },
